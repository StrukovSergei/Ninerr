<template>
    ...
    <li class="gig-preview" @click="goToDetail">
        <vueper-slides @click.native="preventClick" ref="vueperslides1" arrows-outside :touchable="false" fade
            :autoplay="false" @slide="$refs.vueperslides2.goToSlide($event.currentSlide.index, { emit: false })"
            fixed-height="400px">
            <template #arrow-left>
...
export default {
    ...
    watch: {
        gig: {
            handler(newGig) {
                this.slides = newGig.imgUrls.map(imageUrl => ({
                    image: imageUrl
                }))

            },
            immediate: true // Add this if you want to call the handler immediately when the component is created
        },
    },
    methods: {
        goToDetail() {
